...
export const FilmCard_S: React.FC<IFilmCardProps> = (props) => {
  ...
  return (
    <>
          ...
          <Stack justify="space-between" className={styles.titleContainer}>
            <Group justify="space-between" align='flex-start' gap={8} wrap="nowrap">
              <div className={styles.filmInfo}>
                ...
                <Text c='gray.6' my={8}>{props.release_date ? new Date(props.release_date).getFullYear() : 'Unknown'}</Text>
            ...
            {props.isBig ?
              <div className={styles.fullInfoContainer}>
                <Text c='gray.6'>Duration</Text><Text>{getTimeString(props.runtime)}</Text>
                <Text c='gray.6'>Premiere</Text><Text>{new Date(props.release_date).toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: 'numeric'})}</Text>
                <Text c='gray.6'>Budget</Text><Text>${getNumberWithCommas(props.budget) }</Text>
                <Text c='gray.6'>Gross worldwide</Text><Text>${getNumberWithCommas(props.revenue)}</Text>
      ...
      <RateModal isOpen={isRateActive} filmData={props} closeHandler={() => setIsRateActive(false)} />
    </>
  );
};
